...
def sqenisimotomatagawiess_calculate(set_of_molecules, params):
    ...
    for molecule in set_of_molecules.mols:
        ...
        for i, par_index_i in enumerate(molecule.ats_ids):
            ...
            for j, (par_index_j, distance) in enumerate(zip(molecule.ats_ids[i + 1:],
                                                        ...
                                                        i + 1):
                matrix[i, j] = matrix[j, i] = 1.2 / (molecule.distance_matrix[i][j] + 2.4/(params[par_index_i+1] + params[par_index_j+1]))
...
class SQE_Nisimoto_Mataga_Wiess(ChargeMethod):
    ...
    def params_pattern(self):
        return {"atom": {"data": {},
                ...
                "bond": {"data": {},
                         "name": "hardness"},
                "metadata": {"method": "SQE_Nisimoto_Mataga_Wiess"}}
    ...
    def calculate(self, set_of_molecules):
        return sqenisimotomatagawiess_calculate(set_of_molecules, self.params_vals)
