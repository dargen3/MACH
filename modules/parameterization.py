...
from numpy import sum, sqrt, abs, max, linalg, array, linspace, random, arange, mean, empty, isnan, float32, zeros
import heapq
import git
from json import dumps


def local_minimization(input_parameters, minimization_method, method, set_of_molecules, submolecules):
    ...
    res = minimize(calculate_charges_and_statistical_data, input_parameters, method=minimization_method, options={"maxiter": 10000}, bounds=bounds,
                   args=(method, set_of_molecules, submolecules))
...
def prepare_data_for_comparison(method, set_of_molecules, submolecules):
    ...
    for index, atomic_type_charges in enumerate(set_of_molecules.ref_atomic_types_charges):
        ...
        if submolecules:
            asn = set_of_molecules.all_symbolic_numbers_atoms[set_of_molecules.indices]
        else:
...
class Parameterization:
    def __init__(self, sdf, ref_charges, parameters, method, optimization_method, minimization_method, atomic_types_pattern, num_of_molecules, num_of_samples, num_of_candidates, subset_heuristic, validation, cpu, data_dir, rewriting_with_force, submolecules, git_hash=None):
        start_time = date.now()
        files = [(sdf, True, "file"),
        ...
        set_of_molecules = SetOfMolecules(sdf, num_of_molecules=num_of_molecules, parameterization=(ref_charges, validation), submolecules=num_of_submolecules)
        ...
        try:
            ...
            from sys import exit ;exit(colored("ERROR! There are atomic types in last {}% of set of molecules, that are not in first {}%!\n".format(validation, 100 - validation), "red"))
        print(f"Parameterizating by {' '.join(optimization_method.split('_'))}...")
        if optimization_method == "local_minimization":
            _, final_parameters = local_minimization(method.parameters_values, minimization_method, method, set_of_molecules_parameterization, submolecules)
            ...
            partial_f = partial(calculate_charges_and_statistical_data, method=method, set_of_molecules=set_of_molecules_parameterization if subset_heuristic == 0 else SubsetOfMolecules(set_of_molecules_parameterization, method, subset_heuristic, submolecules), submolecules=submolecules)
            print("    Calculating of objective function for samples...")
            with Pool(cpu) as pool:  # evaluate objective function for all samples
            ...
            if subset_heuristic and str(method) in ["EEM", "SFKEEM", "ACKS2", "QEq", "Comba"]:
                partial_f = partial(local_minimization, minimization_method=minimization_method, method=method, set_of_molecules=SubsetOfMolecules(set_of_molecules_parameterization, method, subset_heuristic * 3, submolecules), submolecules=submolecules)
                with Pool(cpu) as pool:
                    main_candidates = [result[1] for result in pool.map(partial_f, [parameters for parameters in main_candidates])]
            partial_f = partial(local_minimization, minimization_method=minimization_method, method=method, set_of_molecules=set_of_molecules_parameterization, submolecules=submolecules)
            with Pool(cpu) as pool: # local minimization for whole set of molecules
        ...
        atomic_types_charges_validation, chg_molecules_validation = prepare_data_for_comparison(method, set_of_molecules_validation, submolecules)
        method.calculate(set_of_molecules_parameterization)
        results_parameterization = method.results
        atomic_types_charges, chg_molecules = prepare_data_for_comparison(method, set_of_molecules_parameterization, submolecules)
        print(colored("ok\n", "green"))
        mkdir(data_dir)
        ...
        write_charges_to_file(charges, method.results, set_of_molecules, submolecules)
